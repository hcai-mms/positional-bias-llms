...
@model_library.register(name="gemini1.5-flash-8b")
class Gemini15Flash8B(BaseModel, weave.Model):
    ...
    async def extract_paragraphs(self, model_output:str, user_prompt:str) -> dict:
        ...
        paragraphs = dict()
        # no meaningful response was given (This model sometimes asks for a list of items even tho the list was given)
        if len(sentences) < 5:
            for t in topics:
                paragraphs[t] = {
                        "position": None,
