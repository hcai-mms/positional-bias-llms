...
@model_library.register(name="phi3-medium")
class Phi3Medium(BaseModel, weave.Model):
    ...
    async def extract_paragraphs(self, model_output:str, user_prompt:str) -> dict:
        ...
        missing_topic_mask = topic_sent_sim.max(axis=1) < 0.5
        topic_sent_sim[missing_topic_mask] = 0
        
        topic_sent_max = topic_sent_sim.argmax(axis=0)
        topic_sent_max[:len(self._header_examples)] = -1 # max to ensure our added headers are never included

        last_valid_idx = len(topic_sent_max)
        ...
        for topic_id in range(len(topics)):
            ...
            if np.abs(np.diff(location)).sum() > 2:
                ...
                for topic_match_idx in topic_matches_indices[1:]:
                    if topic_match_idx - last_idx < 2: # idxs have to be sequential otherwise we cut the segement
